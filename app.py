...
# 2. 식자재 DB (기초 데이터 - 채소류 kg 단위로 정밀화)
if 'ingredient_db' not in st.session_state:
    data = [
        ...
        # 채소류 (kg 단위로 통일 - 정밀 계산용)
        {"품목명": "깐마늘", "규격": "kg", "단가": 8000, "수율": 95},
        {"품목명": "대파", "규격": "kg", "단가": 3500, "수율": 85}, # 단 -> kg 변경
        {"품목명": "양파", "규격": "kg", "단가": 1500, "수율": 90},
        {"품목명": "무", "규격": "kg", "단가": 1200, "수율": 85},    # 개 -> kg 변경
        {"품목명": "배", "규격": "kg", "단가": 5000, "수율": 80},    # 개 -> kg 변경
        {"품목명": "계란(특란)", "규격": "ea", "단가": 350, "수율": 100}, # 계란은 낱개 유지
    ]
    st.session_state.ingredient_db = pd.DataFrame(data)

# 3. 레시피 DB (채소 투입량 g 단위로 변경)
    ...
    st.session_state.recipe_db = [
        {
            ...
            "sub_cat": "국/찌개/전골/탕",
            "ingredients": [
                {"name": "소갈비(Short Rib)", "qty": 250}, # 250g
                {"name": "무", "qty": 150},  # 150g (기존 0.1개 삭제)
                {"name": "대파", "qty": 40},  # 40g (기존 0.1단 삭제)
                {"name": "깐마늘", "qty": 10}, # 10g
                {"name": "진간장", "qty": 15}, # 15ml
            ],
...
with menu_tabs[2]:
    ...
    st.caption("※ 채소류도 중량(g)으로 입력하세요. (시스템이 kg단가를 g단가로 자동 변환합니다)")
...
with menu_tabs[3]:
    ...
    with cost_t2:
        ...
        with col_result_sales:
            ...
            st.metric("총 예상 매출액 (자동 계산)", f"{int(total_expected_sales):,}원")
        ...
        if target_menus:
                    ...
                    for ing in recipe_data['ingredients']:
                        ...
                        if not ing_info.empty:
                            ...
                            calculated_rows.append({
                                ...
                                "1인분 투입량": ing['qty'], "단위(레시피)": unit_display, 
                                "1인분 원가": int(cost)
                            })
            ...
            if calculated_rows:
                ...
                m2.metric("총 예상 마진", f"{int(margin_final):,}원")
